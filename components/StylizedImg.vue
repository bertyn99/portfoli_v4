<template>
    <div class="relative flex aspect-[719/680] w-full ">
        <svg :viewBox="`0 0 ${shapeSelected.width} ${shapeSelected.height}`" fill="none" class="h-full">
            <g :clip-path="`url(#${id}-clip)`" class="group">
                <g class="origin-center scale-100 transition duration-500 motion-safe:group-hover:scale-105">
                    <foreignObject :width="shapeSelected.width" :height="shapeSelected.height">
                        <img alt="" class="w-full bg-neutral-300 object-cover"
                            :style="{ aspectRatio: `${shapeSelected.width} / ${shapeSelected.height}` }"
                            :src="attr.src ?? '/img/laptop.jpg'" v-bind="attr" />
                    </foreignObject>
                </g>
            ...
            <defs>
                <clipPath :id="`${id}-clip`">
                    <path :id="`${id}-shape`" :d="shapeSelected.path" fill-rule="evenodd" clip-rule="evenodd" />
                </clipPath>
            </defs>
        </svg>
    </div>
...
const id = useId()
const shapeSelected = computed(() => shapes[shape])
